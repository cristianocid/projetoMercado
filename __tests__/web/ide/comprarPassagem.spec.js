...
describe('ComprarPassagem', function() {
  ...
  // O teste
  it('ComprarPassagem', async function() {
    await driver.get("https://blazedemo.com/") // abre o site alvo - SUT: Software Under Test
    await driver.manage().window().setRect({ width: 1616, height: 876 }) // tamanho da janela
    ...
    // selecionando o combo de destino e escolhendo a opção Cairo
    {
    ...
    // Carrega a pagina com a lista de voos

    // Seleciona o voo que esta na primeira linha da tabela
    await driver.findElement(By.css("tr:nth-child(1) .btn")).click() // 1 = 1ª linha
    await driver.findElement(By.id("inputName")).click()
    await driver.findElement(By.css(".control-group:nth-child(2) > .control-label")).click()
    
    // Carrega a pagina de pagamento

    // Preencher os campos do formulario

    // Seleciona a Bandeira do cartao (CardType)
    ...
    // Clicar no campo Nome (Name) e escrever cristiano
    await driver.findElement(By.id("inputName")).click()
    await driver.findElement(By.id("inputName")).sendKeys("cristiano") // sendKeys = escrever
    
    await driver.findElement(By.css(".control-group:nth-child(2) > .controls")).click()

    // Clicar no botão Comprar (Purchase) / Botão no final da página
    await driver.findElement(By.css(".btn-primary")).click()

    // Carrega a pagina de agradecimento / confirmação
